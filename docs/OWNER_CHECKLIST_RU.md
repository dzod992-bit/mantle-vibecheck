...
## Приоритет до 15 июня

Делайте строго в таком порядке:

1. Запишите demo video по `docs/VIDEO_SCRIPT_RU.md`.
2. Загрузите видео на YouTube как `Unlisted` или `Public`.
3. Опубликуйте X thread из `docs/X_THREAD.md`.
4. Вставьте обе ссылки в DoraHacks BUIDL.
5. Проверьте preview и отправьте BUIDL до **15 июня 2026 года**.

Пополнение OpenAI API balance не блокирует submission. Сейчас приложение
честно использует рабочий deterministic fallback; live reasoning включится
автоматически после появления API-квоты.
...
6. Нажмите финальную отправку до **15 июня 2026 года**. Не оставляйте это на
   последние часы: точная временная зона закрытия на странице мероприятия не
   указана.
